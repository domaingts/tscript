export default async function (ctx: Ctx) {
    ...
    const mkRow = (children: BaseElement[], gap: number = 4, opts: Partial<StackElement> = {}): StackElement =>
    ...
    const Lunar = {
        ...
        parse(y: number, m: number, d: number) {
    ...
    let currentTerm = "", upcomingTerms: Array<string> = [];
    ...
    const buildRow = (icon: string, color: Color | AdaptiveColor, label: string, content: string, isFirst = true, contentColor = C.sub) => ({
    ...
    const buildYiJiRows = (raw: string, icon: string, color: Color | AdaptiveColor, label: any) => {
        if (!raw) return [];
